...
class CUHK03_LABEL(Dataset):
    ...
    def __init__(self, root, split_id=0, num_val=100, download=True):
        super(CUHK03_LABEL, self).__init__(root, split_id=split_id)
    ...
    def download(self):
        ...
        for labeled, detected in zip(
                matdata['labeled'][0], matdata['detected'][0]):  # rm last two camera pairs(which contains extra imgs.)
            labeled, detected = deref(labeled), deref(detected)  # instance * 10
        ...
        meta = {'name': 'cuhk03_label', 'shot': 'multiple', 'num_cameras': 2,
                'identities': identities}
        write_json(meta, osp.join(self.root, 'meta.json'))

        # Save training and test splits
        splits = []
        view_counts = [deref(ref).shape[0]
                       for ref in matdata['labeled'][0]]  # 843, 440, 77
        vid_offsets = np.r_[0, np.cumsum(view_counts)]  # [0, 843, 1283, 1360]

        # here, matdata['testsets'][0] :20 x 100 x 2
        # 20: 20 test splits
        # 100: every test split contain 100 identities
        # 2: (Camera_id, Identity_id)
        for ref in matdata['testsets'][0]:
            test_info = deref(ref).astype(np.int32)  # 100 x 2
            test_pids = sorted(
